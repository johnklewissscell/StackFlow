...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function updateStock(symbol, range = "1M") {
    ...
    const chartData = isLine
      ? candles.map((d) => ({ x: d.x, y: d.c }))
      : candles;
        ...
        datasets: [
          {
            ...
            fill: isLine,
            // Candlestick styles (Specific to chartjs-chart-financial)
            color: {
              up: "#26a69a",
              down: "#ef5350",
              unchanged: "#999",
  ...
  document.getElementById("toggleCandle").addEventListener("click", (e) => {
    ...
    e.target.innerText = preferCandlestick
      ? "Switch to Line"
      : "Try Candlestick";
  ...
  async function addStockToPortfolio(portEl) {
    ...
    if (!price) {
      if (symbol === "SECRETSAUCE") {
        ...
        const manualPrice = prompt(
          "Market closed or symbol unknown. Enter cost per share:",
        );
    ...
    const currentBalance = parseCurrency(
  ...
  function createPortfolio(name = "Portfolio", bal = 10000) {
    ...
    port.style =
      "background: #fff; border: 1px solid #ccc; padding: 20px; margin-bottom: 30px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);";
    port.innerHTML = `
